    ...
    */
	...
	public:
		...
		void* allocateBytes(size_t byteCount)
		{
			...
			Audit::Assert(size <= (m_disposableOffset - m_offset),
                L"Arena over capacity!");
		...
		void RegisterDisposable(mbuf::Disposable& pDisposable)
		{
			auto pDisSz = static_cast<uint32_t>(sizeof(mbuf::Disposable*));
			Audit::Assert(pDisSz <= (m_disposableOffset - m_offset),
                L"Arena over capacity!");
			Audit::Assert(IsInArena((void*)&pDisposable),
                L"only can register pDisposable object allocated on this arena.");

			m_disposableOffset = m_disposableOffset - pDisSz;

			auto pPDisp = (mbuf::Disposable**)(m_disposableOffset + (char*)m_arenaBytes);
			*pPDisp = &pDisposable;
		}
    ...
    public:
        ...
        // when shutdown
        //
        virtual void RegisterDisposable(mbuf::Disposable* pRes) = 0;
    };
    ...
    public:
        ...
        {
            Audit::Assert(0 <= index && index < ContextIndex::ContextSize,
                L"context index must be within range");
            Audit::Assert(m_multiContext[index] != nullptr,
                L"Context must be set before retrieving");
        ...
        // when shutdown
        //
        void RegisterDisposable(mbuf::Disposable* pRes)
        {
    ...
    //
    class ContinuationBase : mbuf::Disposable
    {
    private:
    ...
    // Template for composing more readable code. Now you can define your
    // continuation right inside the function which started it, with a 
    // lambda function. 
    // Bear in the mind the lambda will execute asychornously, so becareful
    // on what you capture and make sure the reference does not go out of
    // scope!
    //
    template<class Func>
    class ContinuationLambda : public ContinuationBase
    {
        Func f_;
    public:
        ContinuationLambda(Func f, Activity& activity)
        ...
        {
            f_(thread, continuationHandle, length);
        }
    };
    ...
    //
    class EndContinuation : public Continuation < mbuf::BufferEnvelope >
    {
    public:
    ...
    // Stop the activity from outside of the activity
    // RequestShutdown is only effictive if called by one of the
    // continuations.
    //
    inline void StopActivity(Activity& act)
    {
        auto stopper = act.GetArena()->
            allocate<EndContinuation>(act);
        stopper->Post(0, 0);
    }
    ...
    {
    public:
        static const size_t ARENA_SIZE = 4 * SI::Ki;
        static const uint64_t KEY_PRIMARY_ACTION = 1;
